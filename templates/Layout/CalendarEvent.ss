       		...
       		<div class="blog blog-single">
                    ...
                    <div class="blog-info news-listing category-a">
                        ...
                        <ul class="inline medium-12 column">
	                        ...
	                        $Start.format(l), <span class="num">$Start.format(M j)<span>$Start.format(S)</span> <% if not $HideStartAndEndTimes %>$Start.format(g:ia)<% end_if %></span>
			            	<% if OneDay %>
			            		<% if not $HideStartAndEndTimes %> - $End.format(g:ia)<% end_if %>
			            	<% else %>
			            		 - $End.format(l), <span class="num">$End.format(M j)<span>$End.format(S)</span> <% if not $HideStartAndEndTimes %>$End.format(g:ia)<% end_if %></span>
			            	<% end_if %>
							</li>
